...
def experiment_epochs():
    ...
    for epochs in epoch_configs:
        plt.plot(range(1, epochs + 1), results[epochs]['losses'], 
                label=f'Epochs={epochs}', marker='o')
    plt.xlabel('Epoch')
    plt.ylabel('Training Loss')
    ...
    plt.subplot(1, 3, 2)
    for epochs in epoch_configs:
        plt.plot(range(1, epochs + 1), results[epochs]['accuracies'], 
                label=f'Epochs={epochs}', marker='o')
    plt.xlabel('Epoch')
    plt.ylabel('Test Accuracy (%)')
...
def main():
    ...
    for bs, result in bs_results.items():
        print(f"   BS={bs}: Final Accuracy = {result['final_accuracy']:.2f}%, "
